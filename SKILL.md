...
def add_hl(page, needle, color=(1.0, 0.9, 0.3), opacity=0.5, max_n=3):
    n = 0
...
- **透明度默认用 0.5（半透明），不要用 1.0**：高亮注释在多数阅读器
  （Adobe/Chrome）里以正片叠底画在文字下方，字照常显示；但部分阅读器
  （微信内置预览、部分 WPS 版本）不支持混合模式，会把 100% 不透明的
  高亮矩形直接盖在文字上方——字形被遮成白色碎片，看起来像「字画在了
  高亮上」。半透明在所有阅读器下都不可能盖字。
- 如果 PDF 有彩色插图，半透明（0.5）也能让原图透出
